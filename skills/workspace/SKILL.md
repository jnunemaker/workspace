...
- Generic Git worktrees are registered so cleanup still works after an external tool deletes their directories. Run `bin/workspace prune` from a surviving checkout to reconcile immediately; normal bootstrap/run/archive flows also prune automatically.
