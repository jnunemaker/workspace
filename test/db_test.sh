...
patch_database_yml 2>/dev/null
assert_equal "version-one helper is replaced exactly once" "1" "$(grep -c 'workspace_identity_helper_version = 2' config/database.yml)"
...
rendered=$(DATABASE_NAME="custom_development" CONDUCTOR_ROOT_PATH="$TEST_TMP/root-checkout" CONDUCTOR_WORKSPACE_NAME="provider-name" ruby -rerb -e 'print ERB.new(File.read("config/database.yml")).result')
...
YAML
  ...
  rendered=$(CONDUCTOR_ROOT_PATH="$TEST_TMP/root-checkout" CONDUCTOR_WORKSPACE_NAME="provider-name" ruby -rerb -e 'print ERB.new(File.read("config/database.yml")).result')
  printf '%s' "$rendered" > "$rendered_path"
  assert_true "newline-terminated Conductor marker overrides stale Workspace marker" grep -q 'database: "myapp_development_legacy-conductor-name"' "$rendered_path"
...
YAML
  ...
  git_db_root=$(create_fake_app "git-database-root")
  git_db_worktree="$TEST_TMP/git-database-worktree"
  cat > "$git_db_root/config/database.yml" <<'YAML'
development:
  database: git_app_development
test:
  database: git_app_test
YAML
  git -C "$git_db_root" init -q
  git -C "$git_db_root" config user.email "workspace-tests@example.com"
  git -C "$git_db_root" config user.name "Workspace Tests"
  git -C "$git_db_root" add Gemfile config/database.yml
  git -C "$git_db_root" commit -qm "initial"
  git -C "$git_db_root" worktree add -q --detach "$git_db_worktree"

  cd "$git_db_root"
  patch_database_yml 2>/dev/null
  printf '%s' "stale-root-marker" > .workspace
  rendered=$(ruby -rerb -e 'print ERB.new(File.read("config/database.yml")).result')
  printf '%s' "$rendered" > "$rendered_path"
  parsed_database=$(ruby -ryaml -e 'puts YAML.safe_load(File.read(ARGV.fetch(0)), aliases: true).fetch("development").fetch("database")' "$rendered_path")
  assert_equal "generic Git root ignores a stale marker" "git_app_development" "$parsed_database"

  cd "$git_db_worktree"
  patch_database_yml 2>/dev/null
  printf '%s' "stable-git-sibling" > .workspace
  rendered=$(ruby -rerb -e 'print ERB.new(File.read("config/database.yml")).result')
  printf '%s' "$rendered" > "$rendered_path"
  parsed_database=$(ruby -ryaml -e 'puts YAML.safe_load(File.read(ARGV.fetch(0)), aliases: true).fetch("development").fetch("database")' "$rendered_path")
  assert_equal "generic Git sibling uses its stable marker" "git_app_development_stable-git-sibling" "$parsed_database"
