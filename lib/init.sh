...
_remove_legacy_codex_hook() {
  _legacy_file=.codex/hooks.json
  [ -e "$_legacy_file" ] || return 0
  _linked_provider_config "$_legacy_file" && return 0

  _legacy_expected=$(mktemp .codex/.workspace-legacy-hook.XXXXXX) || return 1
  cat > "$_legacy_expected" <<'EOF'
{
...
EOF
  if cmp -s "$_legacy_expected" "$_legacy_file"; then
    rm -f "$_legacy_file" "$_legacy_expected"
    ok "Removed legacy .codex/hooks.json"
  else
    rm -f "$_legacy_expected"
    step ".codex/hooks.json is custom (left unchanged)"
  fi
}
...
# Codex owns cleanup through environment.toml. Remove only the byte-exact file
# generated by older Workspace releases; every other hook source is foreign.
_remove_legacy_codex_hook
